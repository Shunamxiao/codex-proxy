...
```
Read: .Codex/skills/pr-push/references/commit-conventions.md
Read: .Codex/skills/pr-push/references/changelog-format.md
Read: .Codex/skills/pr-push/references/pr-checklist.md
```
...
- **Memory / docs touch** — if the change touches public protocol, request flow, auth lifecycle, or release infra, ask whether `AGENTS.md` needs updating in the same PR.
...
2. **Hook presence check.** The repo's pre-push gate lives at `.Codex/hooks/pre-push-validate.sh`, which is gitignored — fresh clones do **not** have it. Run `test -x .Codex/hooks/pre-push-validate.sh`:
...
1. Read the PR body template:
   ```
   Read: .Codex/skills/pr-push/assets/pr-body-template.md
   ```
